...
function DofHandler(mesh::PolytopalMesh{dim,T}, element::VirtualElement) where {dim,T}
    dofhandler = DofHandler{dim,T}([:u], Int[], Int[], mesh)
    _distribute_dofs(dofhandler, element)
end
...
function field_offset(dh::DofHandler, field::Symbol)
    offset = 0
...
function _distribute_dofs(dh::DofHandler{dim,T}, element::VirtualElement) where {dim,T,shape}
    ...
    facedicts = [Dict{EdgeIndex,Int}() for _ in 1:nvariables(dh)]
